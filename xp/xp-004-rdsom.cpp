...
void setup_options(int argc, char **argv)
{
  ...
  desc.add_options()
    ...
    ("som_sig", po::value<TParam>(&_opt_sig_som)->default_value(_opt_sig_som), "som sigma distance")    
...
Problem load_hmm( const std::string& filename )
{
  ...
  if( pfile.fail() ) {
    std::cout << "load_hmm: NOT FOUND " << filename << std::endl;
    exit(1);
  }
...
Traj load_traj( const std::string& filename )
{
  ...
  auto pfile = std::ifstream( filename );
  if( pfile.fail() ) {
    std::cout << "load_traj: NOT FOUND " << filename << std::endl;
    exit(1);
  }
...
RDSOM load_rdsom( const std::string& filename )
{
  ...
  if( ifile.fail() ) {
    std::cout << "load_rdsom: NOT FOUND " << filename << std::endl;
    exit(1);
  }
 ...
 *
 * Can be used to test without learning
 */
void step_learn( RDSOM& rdsom,
		 const Traj::iterator::difference_type& length,
                 ...
                 const bool learning )
{
  ...
  for( unsigned int i = 0; i < length; ++i) {
    ...
    if( _opt_verb ) {
      std::cout << "__STEP LEARN _nb_step=" << _nb_step;
...
int main(int argc, char *argv[])
{
  ...
  if( _opt_graph ) {
    ...
    save_figseq( filename_png.str(), "FigSeq", false );
